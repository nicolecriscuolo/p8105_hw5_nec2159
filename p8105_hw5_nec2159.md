...
The plot indicates that the probability of at least two people sharing a
birthday becomes greater than 50% when there are more than 23 people in
the room. Additionally, if there are more than 50 people in the room, we
can be almost certain that there will be at least one repeated birthday.
...
When the true mean is 0, the graph shows that we reject the null (𝐻:𝜇=
...
where the estimates were large enough (or small enough in the true mean
equals 0 case) to conclude that the true mean isn’t actually 0. This is
not aligned with the true mean because all of the estimates that were
close to 0, 1, 2, or 3 got filtered out since the null wasn’t rejected
in those cases. As the true mean increases, the average mean estimates
also increase on average. Therefore, the cases where the null gets
rejected here are more aligned with the true mean because estimates
close to 4, 5, and 6 are almost always rejected and are taken into
...
The raw data includes information about homicides in U.S. cities and
includes 52179 observations and 12 variables. It includes information
about the victim such as name, race, age, and sex, information about the
city such as latitude, longitude, and state, and information about the
homicides such as disposition and reported date.
...
<img src="p8105_hw5_nec2159_files/figure-gfm/unnamed-chunk-13-1.png" width="90%" />

The estimated proportion of unsolved homicides is the lowest for
Richmond, VA at around .26 and highest for Chicago, IL at around .74.
Additionally, one observations was omitted for clarity, as the city was
inputted as Tulsa with the state inputted as Alabama. It is unclear if
it should have been included for Tulsa, OK or Birmingham, AL.
